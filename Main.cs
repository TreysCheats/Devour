...
namespace DevourMono
{
    public class Main : MonoBehaviour
    {
	    ...
	    m.serverNameText.text = "Spoofed";
	    m.steamName = "Spoofed";
	    FindObjectOfType<NolanRankController>().SetRank(69);
        }
        public void Update()
        {
            InputManager();
	...
	{
	    Survival Sur = FindObjectOfType<Survival>();
	    try
            {
                Sur.PlayWinEnding("Win");
                Sur.TriggerWinEnding();
            }
            catch
            {
                try
                ...
                catch
                {
                    try
                    {
                        Sur.PlayWinEnding("AsylumWin");
                        Sur.TriggerWinEnding();
                    }
                    catch
                    {
                        Sur.PlayWinEnding("InnWin");
                        Sur.TriggerWinEnding();
                    }
                }
            }
